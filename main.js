// Simple Space Invaders implementation (HTML5 Canvas)
// Controls: Left/Right or A/D, Space to shoot, R to restart
...
(() => {
  ...
  function resetGame() {
    game = {
      ...
      invaders: [],
      invaderDirection: 1,
      invaderSpeed: 30, // pixels per second baseline
      lastShotAt: 0,
      shootCooldown: 300, // ms
      lastInvaderStep: 0,
      invaderStepInterval: 600, // ms between moves (will get faster)
  ...
  function playerShoot() {
    ...
    game.lastShotAt = now;
    const p = game.player;
    game.bullets.push(Bullet(p.x, p.y - 12, -520, 'player'));
  }

  function invaderShoot() {
    if (game.invaderBullets.length > 3) return; // limit simultaneous invader bullets
    ...
    // fire
    game.invaderBullets.push(Bullet(shooter.x + shooter.w / 2, shooter.y + shooter.h + 8, 180, 'invader'));
  }
  ...
  function update(dt) {
    ...
    const now = performance.now();
    if (now - game.lastInvaderStep > game.invaderStepInterval) {
      // compute bounds for alive invaders
      const alive = game.invaders.filter(i => i.alive);
      if (alive.length === 0) {
        game.state = 'won';
        stateEl.textContent = 'You Win! Press R to play again.';
        return;
      }
      ...
      let moveX = 18 * game.invaderDirection;
      // hit side?
      if (minX + moveX < 8 || maxX + moveX > W - 8) {
        // move down instead and reverse
        for (let inv of alive) inv.y += 20;
        game.invaderDirection *= -1;
        // speed up slightly and reduce interval
        game.invaderStepInterval = Math.max(120, game.invaderStepInterval * 0.97);
      } else {
        for (let inv of alive) inv.x += moveX;
      }
      game.lastInvaderStep = now;
      // occasionally shoot
      if (Math.random() < 0.55) invaderShoot();
    }
    ...
    // Win condition handled earlier when no alive invaders
  }
